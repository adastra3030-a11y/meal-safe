...
function read() { try { return { ...defaultState, ...JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}') }; } catch { return { ...defaultState }; } }
...
function setView() { $('#welcome').classList.toggle('hidden', state.started); $('#home').classList.toggle('hidden', !state.started); $('#open-settings').style.visibility = state.started ? 'visible' : 'hidden'; }
function render() {
  setView();
...
function renderAllergens() { $('#common-allergens').innerHTML = common.map(item => `<button type="button" class="chip ${state.allergies.includes(item) ? 'active' : ''}" data-allergen="${item}">${state.allergies.includes(item) ? '✓' : '＋'} ${item}</button>`).join(''); $('#selected-allergens').innerHTML = state.allergies.length ? state.allergies.map(item => `<span class="chip">${escapeHTML(item)} <button type="button" data-remove="${escapeHTML(item)}">×</button></span>`).join('') : '<span class="dialog-note">暂未添加</span>'; }
function saveMeal(event) { event.preventDefault(); const name = $('#meal-name').value.trim(); if (!name) return toast('请填写吃了什么'); state.meals.push({ id: crypto.randomUUID(), date: activeDate, type: $('#meal-type').value, time: $('#meal-time').value, name, ingredientsText: $('#meal-ingredients').value.trim() }); persist(); $('#meal-dialog').close(); render(); toast('已记录这一餐'); }
...
$('#add-allergen').onclick = () => { const field = $('#custom-allergen'), item = field.value.trim(); if (item && !state.allergies.includes(item)) { state.allergies.push(item); field.value = ''; renderAllergens(); } };
document.addEventListener('click', event => { const close = event.target.dataset.close; if (close) $('#' + close).close(); const allergen = event.target.dataset.allergen; if (allergen) { state.allergies = state.allergies.includes(allergen) ? state.allergies.filter(item => item !== allergen) : [...state.allergies, allergen]; renderAllergens(); } const remove = event.target.dataset.remove; if (remove) { state.allergies = state.allergies.filter(item => item !== remove); renderAllergens(); } const mealId = event.target.dataset.id; if (mealId && event.target.classList.contains('delete-meal') && confirm('删除这条餐食记录吗？')) { state.meals = state.meals.filter(item => item.id !== mealId); persist(); render(); } });
render();
